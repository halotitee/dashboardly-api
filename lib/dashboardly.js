...
class DashboardlyDataLoader {
  ...
  getAllBookmarksForBoard(boardId) {
    const page = Number(boardId.page) || 1;
    const limit = Number(boardId.limit) || 9;
    const offset = (page - 1) * limit;      //it's an array, start from [0],shows where the content starts
  ...
  bookmarkBelongsToUser(bookmarkId, userId) {
    return this.query(
      knex
        ...
        .where({
          'bookmarks.id': bookmarkId,               //to specify the ID is bookmarks's id
          ownerId: userId,
        })
        .toString()
    )
  ...
  updateBookmark(bookmarkId, bookmarkData) {
    ...
    return this.query(
      knex('bookmarks')
        .update(util.filterKeys(BOOOKMARK_WRITE_FIELDS, bookmarkData))
        .where('id', bookmarkId)
        .toString()
    )
      .then(() => {
